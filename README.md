...
2. 범용 템플릿 목록은 [TEMPLATE-CATALOG.md](./TEMPLATE-CATALOG.md)에서 확인한다.
3. 선택한 도구 폴더에서 `SETUP-GUIDE.md`로 환경을 만들고 `TEST-GUIDE.md`로 실행 절차를 따른다.
4. 새 프로젝트는 기존 템플릿을 복제해서 시작하고, 공통화 가능한 수정은 다시 템플릿에 환류한다.
5. 테스트 산출물은 각 도구 폴더의 `reports/`에 프로젝트명과 날짜 기준으로 정리한다.
6. 실제 배포 파일, Compose, YAML, 스크립트는 각 도구의 `infra/`, `scripts/`, `plans/`, `scenarios/`에 넣는다.
7. 버전은 반드시 고정한다. 실제 구현 시점의 도구 버전과 Java 버전, 이미지 태그를 배포 파일에 명시한다.
